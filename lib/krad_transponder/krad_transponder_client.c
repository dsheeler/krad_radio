...
static void kr_transponder_info_from_ebml(kr_ebml *ebml, kr_xpdr_info *info) {
...
static int kr_transponder_crate_get_string_from_transponder(kr_crate_t *crate, char **string, int maxlen) {
  ...
  kr_transponder_info_from_ebml(&crate->payload_ebml, &info);

  len += sprintf (*string + len, "Transponder Status:\n");
  len += sprintf (*string + len, "Active Paths: %d", info.active_paths);
