...
<div class='chooser'><div id='MIXER' class='chooser_choice chooser_chosen'>MIXER</div>\
<div id='COMPER' class='chooser_choice'>COMPER</div><div><br clear='both'>\
</div>");
  ...
  $('.chooser_choice').bind('click', function(event) {
    ...
    if (event.currentTarget.id == 'COMPER') {
      $('#kradmixer').fadeTo("slow", 0, function() {
        $('#kradmixer').css({display: 'none'});
         $('#kradcompositor').css({display: 'inline'});
        $('#kradcompositor').fadeTo("slow", 1);
      });
    } else {
        $('#kradcompositor').fadeTo("slow", 0, function () {
          $('#kradcompositor').css({display: 'none'});
          $('#kradmixer').css({display: 'inline'});
          $('#kradmixer').fadeTo("slow", 1);
        });
    }
  });
...
Kr.prototype.portgroup_handle_xmms2 = function (portgroup_name, xmms2) {
  if (xmms2 == 0) {
    if ($('#' + portgroup_name + '_xmms2_wrap')) {
      ...
      + "_xmms2_wrap' class='xmms2_control'><h2>" + portgroup_name + "<div id='"
...
Kr.prototype.toggle_show_effects = function(portgroup_name) {
  var cappa = $('.kradmixer_cappa');
  cappa.toggle();

  if ( $('.effect_area')) {
    $('.effect_area').css({display: 'none'});
  }

  if ($('.eq_area')) {
    $('.eq_area').css({display: 'none'});
  }
  
  if (portgroup_name) {
    this.open_portgroup = portgroup_name;
    var fx = document.id(portgroup_name + "_effects");
    fx.toggle();
    var eq = document.id("eq_" + portgroup_name);
    eq.toggle();
  }
}
...
Kr.prototype.got_add_portgroup = function(crate) {
  ...
  volume_div = "<div class='kradmixer_control volume_control' id='portgroup_" 
   ...
   + "<div class='handle'></div></div></div><br><div class='button_wrap'>"
   + "<div class='krad_button_toggle' id='fx_"
   + crate.portgroup_name + "'>Fx</div></div>"
   +"<div class='button_wrap'>"
   + "<div class='krad_button_toggle' id='fade_"
   + crate.portgroup_name + "'>Fade</div></div>" 
   + "<br clear='both' /></div>";
  ...
  if (crate.direction == 1) {
    ...
    $('.kradmixer_cappa').append(
     ...
     + "'><br clear='both'/><h2>Eq for " + crate.portgroup_name
     + "</h2><h2 id='eq_description_" + crate.portgroup_name + "'>Result:</h2></div>");
