...
Kr.prototype.load_interface = function() {
  ...
  this.interface_data.ui_chosen = 'MIXER';
...
Kr.prototype.got_sysname = function(sysname) {
  ...
  $('#kradradio').append("<div class='kradradio_station' id='"
   + this.sysname + "'>\
<div id='kradmixer' class='kradmixer'>\
<div id='mixers'><div class='chooser'></div></div>\
<div class='kradmixer_cappa'><button class='close'>&times;</button></div>\
<div id='kradcompositor' class='kradcompositor'></div>\
<div id='crossfades'><button class='close'>&times;</button></div>\
<div id='xmms2s'><button class='close'>&times;</button></div>\
</div>\
</div>");
...
<div id='COMPER' class='chooser_choice'><h2>COMPER</h2>\
  ...
  $("#crossfades .close").bind('click', function () {
    $('#mixers .chooser').append("<div class='chooser_choice' id='crossfades_min_choice'"
     + "><h2>Crossfades</h2></div>");
     $('#crossfades').hide(1000);

    $('#crossfades_min_choice').bind('click', function() {
      $('#crossfades').show(1000);
      $(this).remove();
    });
  });

    $("#xmms2s .close").bind('click', function () {
    $('#mixers .chooser').append("<div class='chooser_choice' id='xmms2s_min_choice'"
     + "><h2>Xmms2 Controls</h2></div>");
     $('#xmms2s').hide(1000);

    $('#xmms2s_min_choice').bind('click', function() {
      $('#xmms2s').show(1000);
      $(this).remove();
    });
  });
  
  $('#kradcompositor').append("<button class='close'>&times;</button>"
   + "<canvas width='960px' height='540px' class='kradcompositor_canvas'"
   + "id='compositor_canvas'>"
   + "</canvas><div><ul id='subunit_list' class='kradcompositor_subunit_list'>"
   + "</ul></div>");

  $("#kradcompositor .close").bind('click', function () {
    $('#mixers .chooser').append("<div class='chooser_choice' id='kradcompositor_min_choice'"
     + "><h2>Comper</h2></div>");
     $('#kradcompositor').hide(1000);

    $('#kradcompositor_min_choice').bind('click', function() {
      $('#kradcompositor').show(1000);
      $(this).remove();
    });
  });
  ...
  $('#xmms2s, #crossfades').mouseenter(kr.mouseenter_glow);
  $('#xmms2s, #crossfades').mouseleave(kr.mouseleave_restore);
...
//	  $('#portgroup_' + portgroup_name + '_wrap').after("<div id='portgroup_"
    $('#crossfades').append("<div id='portgroup_"
     + portgroup_name + "_crossfade_wrap' " 
     + "class='kradmixer_control crossfade_control'> <h2>" + portgroup_name
     + " - " + crossfade_name + "</h2><div id='"
...
Kr.prototype.portgroup_handle_xmms2 = function (portgroup_name, xmms2) {
  if (xmms2 == 0) {
    if ($('#' + portgroup_name + '_xmms2_wrap')) {
     ...
     + portgroup_name + "_xmms2'></div></div><br clear='all'/>";
...
Kr.prototype.toggle_show_effects = function(portgroup_name) {
  $('.kradmixer_cappa').fadeToggle(1000);
  $('.effect_area').hide(1000);
  $('.eq_area').hide(1000);
  
  if (portgroup_name) {
    $('#' +portgroup_name + "_effects") .show(1000);
    $('#eq_' + portgroup_name).show(1000);
  }
}
...
Kr.prototype.got_add_portgroup = function(crate) {
  ...
  volume_div = "<div class='kradmixer_control volume_control' id='portgroup_" 
   + crate.portgroup_name + "_wrap'><button class='close'>&times;</button>"
   + "<h2>" + crate.portgroup_name 
   + "</h2><div class='meter_wrapper'>"
   + "<canvas width='10' height='200' class='meter' id='" 
   + crate.portgroup_name + "_meter_canvas'></canvas></div><div id='" 
   + crate.portgroup_name + "'><div class='volume'>"
   + "<div class='handle'></div></div></div>"
   +"<div class='button_wrap'><div class='krad_button3' id='fade_"
   + crate.portgroup_name + "'>Fade</div></div><div class='button_wrap'>"
   + "<div class='krad_button3' id='fx_"
   + crate.portgroup_name + "'>Fx</div></div>"
   +"</div>";

  $('#mixers').append(volume_div);

  $("#portgroup_" + crate.portgroup_name + "_wrap .close").bind('click', function () {
    $('#mixers .chooser').append("<div class='chooser_choice' id='portgroup_"
     + crate.portgroup_name + "_min_choice'><h2>"
     + crate.portgroup_name + "</h2></div>");
     $('#portgroup_' + crate.portgroup_name + '_wrap').hide(1000);

    $('#portgroup_' + crate.portgroup_name + '_min_choice').bind('click', function() {
      $('#portgroup_' + crate.portgroup_name + '_wrap').show(1000);
      $(this).remove();
    });
  ...
  if (crate.portgroup_name == "DTMF") {
    $('#portgroup_' + crate.portgroup_name + "_wrap").append("<div class='button_wrap'>"
     + "<div class='krad_button3' id='dtmf_toggle'>Dial Pad</div></div>");

	  $('#kradmixer').append("<div class='kradmixer_control dtmf_pad'>" 
     + " <div id='" + crate.portgroup_name + "_dtmf'> </div> </div>");

    $('#dtmf_toggle').bind('click', function () {
      $('.dtmf_pad').fadeToggle();
    });
  ...
  if (crate.direction == 1) {
     ...
     + crate.portgroup_name + "</h2><h3 id='effect_description_" + crate.portgroup_name
     + "'>Info:</h3></div>");
    ...
    this.interface_data.portgroup_effect_sliders[name] = new Slider(effect_slider,
     effect_slider.getElement('.handle'), {
       ...
       onChange: function (value) {
         kr.ctrl("e/" + crate.portgroup_name + "/lp/hz", value);
         $('#effect_description_' + crate.portgroup_name).replaceWith(
           "<h3 id='effect_description_" 
           + crate.portgroup_name + "'>lowpass freq: " + value + "</h3>");
       }
     });
    ...
    this.interface_data.portgroup_effect_sliders[name] = new Slider(effect_slider, 
     effect_slider.getElement('.handle'), {
       range: [5.0, 0],
        ...
        onChange: function (value) {
          kr.ctrl("e/" + crate.portgroup_name + "/lp/bw", value);
          $('#effect_description_' + crate.portgroup_name).replaceWith(
           "<h3 id='effect_description_" 
           + crate.portgroup_name + "'>lowpass bandwidth: " + value + "</h3>");
        }
     });
    ...
    this.interface_data.portgroup_effect_sliders[name] = new Slider(effect_slider, effect_slider.getElement('.handle'), {
      range: [20000, 20],
        ...
        onChange: function (value) {
          kr.ctrl("e/" + crate.portgroup_name + "/hp/hz", value);
          $('#effect_description_' + crate.portgroup_name).replaceWith(
           "<h3 id='effect_description_" 
           + crate.portgroup_name + "'>highpass freq: " + value + "</h3>");
        }
     });
    ...
    this.interface_data.portgroup_effect_sliders[name] = new Slider(effect_slider, effect_slider.getElement('.handle'), {
      range: [5, 0],
        ...
        onChange: function (value) {
          kr.ctrl("e/" + crate.portgroup_name + "/hp/bw", value);
          $('#effect_description_' + crate.portgroup_name).replaceWith(
           "<h3 id='effect_description_" 
           + crate.portgroup_name + "'>highpass bandwidth: " + value + "</h3>");
        }
     });
    ...
    this.interface_data.portgroup_effect_sliders[name] = new Slider(effect_slider, effect_slider.getElement('.handle'), {
      range: [10, 0],
        ...
        onChange: function (value) {
          kr.ctrl("e/" + crate.portgroup_name + "/a/drive", value);
          $('#effect_description_' + crate.portgroup_name).replaceWith(
           "<h3 id='effect_description_" 
           + crate.portgroup_name + "'>drive: " + value + "</h3>");
        }
     });
    ...
    this.interface_data.portgroup_effect_sliders[name] = new Slider(effect_slider, effect_slider.getElement('.handle'), {
      range: [10, -10],
        ...
        onChange: function (value) {
          kr.ctrl("e/" + crate.portgroup_name + "/a/blend", value); 
          $('#effect_description_' + crate.portgroup_name).replaceWith(
           "<h3 id='effect_description_" 
           + crate.portgroup_name + "'>blend: " + value + "</h3>");
        }
    });
    $('.kradmixer_cappa').append(
     "<div class='eq_area' id='eq_" + crate.portgroup_name
     + "'><h2>Eq for " + crate.portgroup_name
     + "</h2><h3 id='eq_description_" + crate.portgroup_name + "'>Info:</h3></div>");
    ...
    for (var i = 0; i < 32; i=i+4) {
      ...
      this.interface_data[name] = new Slider(eq_slider,
       eq_slider.getElement('.handle'), {
        ...
        onChange: function (value) {
          kr.ctrl(this.options.address, value);
          $('#eq_description_' + crate.portgroup_name).replaceWith("<h3 id='eq_description_" 
           + crate.portgroup_name + "'>Db: " + value + "</h3>");
        }
	    });
	  }
    /*
  ...
  $('#' + crate.portgroup_name + '_effects, #eq_' + crate.portgroup_name
   +', #portgroup_' + crate.portgroup_name + "_wrap").mouseenter(kr.mouseenter_glow);

  $('#' + crate.portgroup_name + '_effects, #eq_' + crate.portgroup_name
   +', #portgroup_' + crate.portgroup_name + "_wrap").mouseleave(kr.mouseleave_restore);

  $("#" + crate.portgroup_name + " .volume .handle").mouseenter(kr.mouseenter_handle);
  $("#" + crate.portgroup_name + " .volume .handle").mouseleave(kr.mouseleave_handle);
