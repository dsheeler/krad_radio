...
Kr.prototype.load_interface = function() {
  ...
  this.interface_data.portgroup_values = new Object();
  this.interface_data.crossfades = new Object();
  this.interface_data.portgroup_volume_toggle = new Object();
  this.interface_data.opacity_toggle = new Object();
  this.interface_data.grid = new Object();
  this.interface_data.screen = new Object();
  this.interface_data.grid_cell = new Object();
  this.interface_data.rack_control_units = new Array();
  this.interface_data.shown_effects = '';
  this.interface_data.background_color_backups = new Object();
...
rackspace = 80;
  ...
  this.interface_data.grid['width'] = Math.floor(window.innerWidth / rackspace);
  this.interface_data.grid['height'] = Math.floor(window.innerHeight / rackspace);
  var OFF_GRID = 666;
  kr.interface_data.RCUs = [{
        sel: '#kradcompositor',
        width: 8,
        height: 5,
        x: OFF_GRID,
...
Kr.prototype.first_open_rack_position = function(RCU, positioned_RCUs) {
  var ret = null;
  var fits = 1;
  for (var j = 0; j < kr.interface_data.grid.height; j++) {
    for (var i = 0; i < kr.interface_data.grid.width; i++) {
      for (var r = 0; r < positioned_RCUs.length; r++) {
       if (
           ...
           ||
            ...
            && (j + RCU.height>= positioned_RCUs[r].y && positioned_RCUs[r].y + positioned_RCUs[r].height > j+RCU.height))
            ||
             (i+RCU.width > kr.interface_data.grid.width || j + RCU.height > kr.interface_data.grid.height)
           ) {
...
Kr.prototype.position_RCUs = function() {
  ...
  this.interface_data.screen['width'] = window.innerWidth;
  this.interface_data.screen['height'] = window.innerHeight;
  this.interface_data.grid_cell['width'] = this.interface_data.screen['width'] 
   / this.interface_data.grid['width'];
  this.interface_data.grid_cell['height'] = this.interface_data.screen['height'] 
   / this.interface_data.grid['height'];

  var positioned_RCUs = new Array();
 
  $('body').css({'font-size': Math.max(12, Math.round(Math.min(16*window.innerWidth/1920,16*window.innerHeight/1080.0))) + 'px'});
  for (var i = 0; i < kr.interface_data.RCUs.length; i++) {
    

        kr.interface_data.RCUs[i];
        
        var result =kr.first_open_rack_position(kr.interface_data.RCUs[i], positioned_RCUs);
        if (result != null) {
          positioned_RCUs.push(kr.interface_data.RCUs[i]);
          kr.interface_data.RCUs[i].x = result[0];
          kr.interface_data.RCUs[i].y = result[1];
        }
        
        var top = kr.interface_data.grid_cell.height * kr.interface_data.RCUs[i].y;
        var left = kr.interface_data.grid_cell.width * kr.interface_data.RCUs[i].x;
        var height = kr.interface_data.grid_cell.height * kr.interface_data.RCUs[i].height;
        var width = kr.interface_data.grid_cell.width * kr.interface_data.RCUs[i].width;

        $(kr.interface_data.RCUs[i].sel).css({
            'top': top + 'px'
        });
        $(kr.interface_data.RCUs[i].sel).css({
            'left': left + 'px'
...
<div><div class='button_wrap'><div class='krad_button3' id='uber_happy'>\
UBER HAPPY</div></div><ul id='subunit_list' class='RCU kradcompositor_subunit_list'></ul></div>\
...
Kr.prototype.maximized_crossfader_group_selectors = function(input) {
  ...
  if (this.interface_data.crossfades[input]) {
    selectors.push('#' + this.interface_data.crossfades[input][0] + '_crossfade_wrap');
    if (this.interface_data.crossfades[input][0] == input) {
      selectors.push('#portgroup_' + this.interface_data.crossfades[input][1] + '_wrap');
    } else {
      selectors.push('#portgroup_' + this.interface_data.crossfades[input][0] + '_wrap');
    }
  }
  return selectors;
}
...
Kr.prototype.minimized_crossfader_group_selectors = function(input) {
  ...
  if (this.interface_data.crossfades[input]) {
    if (this.interface_data.crossfades[input][0] == input) {
      selectors.push('#portgroup_' + this.interface_data.crossfades[input][1] + '_minimized_wrap');
    } else {
      selectors.push('#portgroup_' + this.interface_data.crossfades[input][0] + '_minimized_wrap');
    }
  }
  return selectors;
}
...
Kr.prototype.portgroup_alt_draw_peak_meter = function (portgroup_name, value) {
 ...
 ctx.save();
  ...
  ctx.stroke();
   ...
   ctx.beginPath();
  ctx.arc (0, 0, 0.1 * size, 0, 2 * Math.PI);
  ctx.fill();*/
    ctx.restore();
  /*
 ...
 control_name, value) {
  if ($('#' + portgroup_name)) {
    if (control_name == "volume") {
      this.interface_data.portgroup_values[portgroup_name] = value;
...
Kr.prototype.portgroup_handle_crossfade_group = function(portgroup_name,
 crossfade_name, crossfade) {
  ...
  if (crossfade_name.length > 0) {
    ...
    this.interface_data.crossfades[portgroup_name] = [portgroup_name, crossfade_name];
    this.interface_data.crossfades[crossfade_name] = [portgroup_name, crossfade_name];
    
    var crossfade_element = document.id(portgroup_name + "_crossfade");
    var crossfade_slider = crossfade_element.getElement('.fade');

    this.interface_data.portgroup_crossfade_sliders[portgroup_name] = 
...
Kr.prototype.got_add_subunit = function (crate) {
  ...
  if ((crate.subunit_type != 'videoport') || (crate.direction != 'output')) {
    ...
    $('#fade_' + type_num).bind("click", function() {
      var new_value = 0;
      if (kr.interface_data.opacity_toggle[type_num] == 0) {
        new_value = 1;
        kr.interface_data.opacity_toggle[type_num] = 1;
      } else {
        new_value = 0;
        kr.interface_data.opacity_toggle[type_num] = 0;
      }
...
Kr.prototype.toggle_show_effects = function(portgroup_name) {
  ...
  if (this.interface_data.shown_effects != portgroup_name) {
    if (this.interface_data.shown_effects != '') {
      $('#' + this.interface_data.shown_effects + "_effects") .hide(200);
      $('#eq_' + this.interface_data.shown_effects).hide(200);
    } else {
    ...
    this.interface_data.shown_effects = portgroup_name;
  } else {
...
Kr.prototype.got_add_portgroup = function(crate) {
  ...
  $("#portgroup_" + crate.portgroup_name + "_wrap .close").bind('mouseenter', {'crate': crate}, function(event) {
    ...
    for (var i = 0; i < selectors.length; i++) {
      kr.interface_data.background_color_backups[selectors[i]] = $(selectors[i]).css('background-color');
   ...
   $("#portgroup_" + crate.portgroup_name + "_wrap .close").bind('mouseleave', {'crate': crate}, function(event) {
    var selectors = kr.maximized_crossfader_group_selectors(event.data.crate.portgroup_name);
    for (var i = 0; i < selectors.length; i++) {
      $(selectors[i]).css('background-color', kr.interface_data.background_color_backups[selectors[i]]);
    }
  });
  ...
  $("#portgroup_" + crate.portgroup_name + "_wrap .close").bind('click', {'crate': crate}, function(event) {
    ...
    for (var i = 0; i < 2; i++) {
      $("#portgroup_" + kr.interface_data.crossfades[event.data.crate.portgroup_name][i] +"_minimized_wrap").append($("#"
       + kr.interface_data.crossfades[event.data.crate.portgroup_name][i] + "_meter_canvas"));

      $("#" + kr.interface_data.crossfades[event.data.crate.portgroup_name][i] + "_meter_canvas").css({'position': 'relative', 
        'display': 'inline'});

      $('#portgroup_' + kr.interface_data.crossfades[event.data.crate.portgroup_name][i]  + '_minimized_wrap').bind('click', {'i': i, 'crate': event.data.crate}, function(e) {
        ...
        for (var j = 0; j < 2; j++) {
          $("#portgroup_" + kr.interface_data.crossfades[e.data.crate.portgroup_name][j] + "_wrap .meter_wrapper" ).append(
           $("#" +  kr.interface_data.crossfades[e.data.crate.portgroup_name][j] + "_meter_canvas"));
          $("#" + kr.interface_data.crossfades[e.data.crate.portgroup_name][j] + "_meter_canvas").css({'top': '0px'});
        }
  ...
  $('#fade_' + crate.portgroup_name).bind("click",
     function(event, ui) {
      var new_value = 0;
      if (kr.interface_data.portgroup_volume_toggle[crate.portgroup_name]) {
        new_value = kr.interface_data.portgroup_volume_toggle[crate.portgroup_name];
        kr.interface_data.portgroup_volume_toggle[crate.portgroup_name] = 0;
      } else {
        new_value = 0;
        kr.interface_data.portgroup_volume_toggle[crate.portgroup_name] 
          = kr.interface_data.portgroup_values[crate.portgroup_name];
      }
  ...
  kr.interface_data.portgroup_values[crate.portgroup_name] = crate.volume; 
  this.interface_data.portgroup_volume_sliders[name] = new Slider(volume_slider,
   volume_slider.getElement('.handle'), {
      range: [100, 0],
        ...
        onChange: function (value) {
         kr.interface_data.portgroup_values[crate.portgroup_name] = value;
