...
<div id='crossfades'></div>\
<div id='xmms2s'></div>\
</div>\
<div id='kradcompositor' style='display: none' class='kradcompositor'></div>\
...
<div id='COMPER' class='chooser_choice'><h2>COMPER</h2>\
  ...
  $('.chooser_choice').bind('click', function(event) {
    $('.chooser_choice').removeClass('chooser_chosen');
    event.currentTarget.addClass('chooser_chosen');
    if (event.currentTarget.id == 'COMPER') {
      $('#kradmixer').fadeOut(1000, function() {
        $('#kradcompositor').fadeIn(1000);
      });
    } else {
      $('#kradcompositor').fadeOut(1000, function () {
        $('#kradmixer').fadeIn(1000);
      });
    }
  });
  
  $('.kradcompositor').append("<div class='kradcompositor_control'><h2>The Comper</h2>"
   + "<canvas width='960px' height='540px' class='kradcompositor_canvas'"
   + "id='compositor_canvas'>"
   + "</canvas><br><ul id='subunit_list' class='kradcompositor_subunit_list'>"
   + "</ul></div>");

  kr.compositor_canvas = new CanvasState(document.getElementById
  ("compositor_canvas"), document.createElement('canvas'));

  $('.close').bind('click', function(e, ui) {
...
Kr.prototype.portgroup_handle_xmms2 = function (portgroup_name, xmms2) {
  if (xmms2 == 0) {
    ...
    $('#' + portgroup_name + '_xmms2').append("<div class='button_wrap'>"
     + "<div class='krad_button_xmms' id='"
     + portgroup_name + "_xmms2_prev'>PREV</div></div>");
    $('#' + portgroup_name + '_xmms2').append("<div class='button_wrap'>" 
     + "<div class='krad_button_xmms' id='" + portgroup_name
...
Kr.prototype.toggle_show_effects = function(portgroup_name) {
  ...
  $('.effect_area').fadeOut(1000);
  $('.eq_area').fadeOut(1000);
  
  if (portgroup_name) {
    $('#' +portgroup_name + "_effects") .fadeToggle(1000);
    $('#eq_' + portgroup_name).fadeToggle(1000);
  }
}
...
Kr.prototype.got_add_portgroup = function(crate) {
  ...
  volume_div = "<div class='kradmixer_control volume_control' id='portgroup_" 
   + crate.portgroup_name + "_wrap'><h2>" + crate.portgroup_name 
   + "</h2><br/><div class='meter_wrapper'>"
   ...
   + "<div class='handle'></div></div></div><br>"
   +"<div class='button_wrap'><div class='krad_button_xmms' id='fade_"
   + crate.portgroup_name + "'>Fade</div></div><div class='button_wrap'>"
   + "<div class='krad_button_xmms' id='fx_"
  ...
  if (crate.direction == 1) {
    ...
    $('.kradmixer_cappa').append(
     ...
     + "</h2><h2 id='eq_description_" + crate.portgroup_name + "'></h2></div>");
    ...
    $('#' + 'eq_' + crate.portgroup_name).append("<br clear='both'/>"
     + " <div id='" + crate.portgroup_name + "_effects_eq_db'"
     + "style='float:left'></div>");
      ...
      this.interface_data[name] = new Slider(eq_slider,
       eq_slider.getElement('.handle'), {
        ...
        onChange: function (value) {
          kr.ctrl(this.options.address, value);
          $('#eq_description_' + crate.portgroup_name).replaceWith("<h2 id='eq_description_" 
           + crate.portgroup_name + "'>Db: " + value + "</h2>");
        }
	    });
	  }
    /*
