Kradradio.prototype.comp_ctrl = function(num, type, control_name, value) {
  var t;
  if (type == "videoport") {
    t = "v";
  } else {
  ...
  kradradio.ctrl(t + "/" + num + "/" + control_name, value);
}
...
function CanvasState(canvas, ghostcanvas) {
  ...
  canvas.addEventListener('mousemove', function(e) {
      ...
      myState.selection.x = mouse.x - myState.dragoffx;
	    ...
	    kradradio.comp_ctrl(myState.selection.num, 
      ...
      kradwebsocket.debug(myState.selection.PCView.handles[
       myState.expectMovePerspectiveCorrectionHandle].x 
       + " " + myState.selection.PCView.handles[
       myState.expectMovePerspectiveCorrectionHandle].y);
      ...
      switch (myState.expectResize) {
          ...
          if (w/h >= myState.selection.aspectRatio) {
	        ...
	        kradradio.comp_ctrl(myState.selection.num, myState.selection.type, 
           "width", w);
	        kradradio.comp_ctrl(myState.selection.num, myState.selection.type, 
           "x", x);
...
Kradradio.prototype.portgroup_draw_peak_meter 
...
Kradradio.prototype.got_update_subunit = function(num, type, 
 control_name, value) {
  for (var i = 0; i < kradradio.compositor_canvas.shapes.length; i++) {
    if (kradradio.compositor_canvas.shapes[i].type == type
     && (kradradio.compositor_canvas.shapes[i].num == num)) {
...
Kradradio.prototype.got_control_portgroup = function(portgroup_name,
 control_name, value) {
  if ($('#' + portgroup_name)) {
  ...
  if (crate.effect_name == "eq") {
    if ($('#' + crate.portgroup_name + '_' + crate.effect_name
     + '_' + crate.effect_num + '_' + crate.control_name)) {
	    $('#' + crate.portgroup_name + '_' + crate.effect_name
       + '_' + crate.effect_num + '_'
       + crate.control_name).slider( "value" , crate.value );
	  }
  } else {
	  if ($('#' + crate.portgroup_name + '_' + crate.effect_name + '_'
     + crate.control_name)) {
	    $('#' + crate.portgroup_name + '_' + crate.effect_name
     + '_' + crate.control_name).slider( "value" , crate.value );
	  }
  }
}

Kradradio.prototype.got_update_portgroup = function (portgroup_name,
 control_name, value) {
...
Kradradio.prototype.portgroup_handle_crossfade_group = function(portgroup_name,
 crossfade_name, crossfade) {
  ...
  if (crossfade_name.length > 0) {
     ...
     + portgroup_name + "_crossfade'></div> <h2>" + portgroup_name
     + " - " + crossfade_name + "</h2></div>");
    $('#' + portgroup_name + '_crossfade').slider(
     {orientation: 'horizontal', value: crossfade, min: -100, max: 100 });

    $('#' + portgroup_name + '_crossfade' ).bind("slide",
     function(event, ui) {
       kradradio.ctrl("m/" + portgroup_name + "/c", ui.value);
	   });
  }
}
...
Kradradio.prototype.portgroup_handle_xmms2 = function (portgroup_name, xmms2) {
  if (xmms2 == 0) {
    ...
    $('#' + portgroup_name + '_xmms2').append("<div class='button_wrap'>" 
     ...
     function(event, ui) {
      kradradio.xmms2(portgroup_name, "prev");
    });

    $('#' + portgroup_name + '_xmms2_play' ).bind("click",
     function(event, ui) {
      kradradio.xmms2(portgroup_name, "play");
    });
	      
	  $('#' + portgroup_name + '_xmms2_pause' ).bind("click",
     function(event, ui) {
	    kradradio.xmms2(portgroup_name, "pause");
	  });
	      
    $('#' + portgroup_name + '_xmms2_stop' ).bind("click",
     function(event, ui) {
	    kradradio.xmms2(portgroup_name, "stop");
	  });
	      
	  $('#' + portgroup_name + '_xmms2_next' ).bind("click",
     function(event, ui) {
	    kradradio.xmms2(portgroup_name, "next");
	  });
  }
}
...
Kradradio.prototype.got_add_subunit = function (crate) {
  if (!document.getElementById("compositor_canvas")) {
    ...
    kradradio.compositor_canvas = new CanvasState(document.getElementById
...
Kradradio.prototype.got_remove_subunit = function(crate) {
  kradradio.compositor_canvas.removeShape(crate.subunit_type,
   crate.subunit_id);
...
Kradradio.prototype.toggle_show_effects = function(portgroup_name) {
  $('#' + portgroup_name + '_effects').toggle("slide", {}, 500);
...
Kradradio.prototype.got_add_portgroup = function(crate) {
  ...
  volume_div = "<div class='kradmixer_control volume_control' id='portgroup_" 
   ...
   + "<canvas width='42' height='220' class='meter' id='" 
  // + crate.portgroup_name + "_meter_canvas'></canvas></div><div id='" 
  // + crate.portgroup_name + "'></div><h2>" + crate.portgroup_name 
  // + "</h2></div>";

   + crate.portgroup_name + "_meter_canvas'></canvas></div><div id='" 
   + crate.portgroup_name + "'></div><div class='button_wrap'><div class='krad_button' id='show_effects_"
  ...
  $('#show_effects_' + crate.portgroup_name).bind("click",
     function(event, ui) {
      kradradio.toggle_show_effects(crate.portgroup_name);
  });
  ...
  $('#' + crate.portgroup_name).slider({orientation:
   'vertical', value: crate.volume, step: 0.5 });

  $( '#' + crate.portgroup_name ).bind( "slide", function(event, ui) {
	  kradradio.ctrl("m/" + crate.portgroup_name + "/v", ui.value);
  });
  ...
  if (crate.crossfade_name.length > 0) {
	  kradradio.portgroup_handle_crossfade_group(crate.portgroup_name,
  ...
  $('#portgroup_' + crate.portgroup_name + '_wrap').append("<div id='ktags_"
   + crate.portgroup_name + "'></div>");
  ...
  if (crate.direction == 1) {
     ...
     + crate.portgroup_name + "_lowpass_hz'></div>");
	  $('#' + crate.portgroup_name + '_effects').append
     ("<div class='kradmixer_control' id='"
     + crate.portgroup_name + "_lowpass_bw'></div>");
	  $('#' + crate.portgroup_name + '_effects').append
     ("<div class='kradmixer_control' id='"
     + crate.portgroup_name + "_highpass_hz'></div>");
	  $('#' + crate.portgroup_name + '_effects').append
     ("<div class='kradmixer_control' id='"
     + crate.portgroup_name + "_highpass_bw'></div>");
	  $('#' + crate.portgroup_name + '_effects').append
     ("<div class='kradmixer_control' id='"
     + crate.portgroup_name + "_analog_drive'></div>");
	  $('#' + crate.portgroup_name + '_effects').append
     ("<div class='kradmixer_control' id='"
     + crate.portgroup_name + "_analog_blend'></div>");
	      
	  $('#' + crate.portgroup_name + '_lowpass_hz').slider({orientation:
     'vertical', value: crate.lowpass_hz, step: 1, min: 20.0, max: 12000.0 });
	  $('#' + crate.portgroup_name + '_lowpass_bw').slider({orientation:
     'vertical', value: crate.lowpass_bw, step: 0.1, min: 0.1, max: 5.0 });
	  $('#' + crate.portgroup_name + '_highpass_hz').slider({orientation:
     'vertical', value: crate.highpass_hz, step: 1, min: 20.0, max: 20000.0 });
	  $('#' + crate.portgroup_name + '_highpass_bw').slider({orientation:
     'vertical', value: crate.highpass_bw, step: 0.1, min: 0.1, max: 5.0 });
	  $('#' + crate.portgroup_name + '_analog_drive').slider({orientation:
     'vertical', value: crate.analog_drive, step: 0.1, min: 0.1, max: 10.0 });
	  $('#' + crate.portgroup_name + '_analog_blend').slider({orientation:
     'vertical', value: crate.analog_blend,
      step: 0.1, min: -10.0, max: 10.0 });
	      
	  $('#' + crate.portgroup_name + '_lowpass_hz' ).bind("slide",
     function(event, ui) {
	    kradradio.ctrl("e/" + crate.portgroup_name + "/lp/hz", ui.value);
	  });

	  $('#' + crate.portgroup_name + '_lowpass_bw' ).bind("slide",
     function(event, ui) {
	    kradradio.ctrl("e/" + crate.portgroup_name + "/lp/bw", ui.value);
	  });
	      
	  $( '#' + crate.portgroup_name + '_highpass_hz' ).bind("slide",
     function(event, ui) {
	    kradradio.ctrl("e/" + crate.portgroup_name + "/hp/hz", ui.value);
	  });
	      
	  $( '#' + crate.portgroup_name + '_highpass_bw' ).bind("slide",
     function(event, ui) {
	    kradradio.ctrl("e/" + crate.portgroup_name + "/hp/bw", ui.value);
	  });

	  $( '#' + crate.portgroup_name + '_analog_drive' ).bind("slide",
     function(event, ui) {
	    kradradio.ctrl("e/" + crate.portgroup_name + "/a/drive", ui.value);
	  });
	      
	  $( '#' + crate.portgroup_name + '_analog_blend' ).bind("slide",
     function(event, ui) {
	    kradradio.ctrl("e/" + crate.portgroup_name + "/a/blend", ui.value);
	  });

	  $('.kradmixer_cappa').append("<br clear='both'/><div class='effect_area'>"
     + " <div id='" + crate.portgroup_name + "_effects_eq' style='float:left'></div> </div>");

    for (var i = 0; i < 32; i++) {
      $('#' + crate.portgroup_name + '_effects_eq').append
       ("<div class='kradmixer_control' id='"
       + crate.portgroup_name + "_eq_" + i + "_hz'></div>");
	    var mini;
	    var maxi;
	    ...
	    $('#' + crate.portgroup_name + '_eq_' + i + '_hz').slider
       ({orientation: 'vertical', value: crate.eq.bands[i].hz, step: 1, min: mini, max: maxi });
	    $( '#' + crate.portgroup_name + '_eq_' + i + '_hz' ).bind
       ("slide", {band: i}, function(event, ui) {
	      kradradio.ctrl("e/" + crate.portgroup_name + "/eq/"
         + event.data.band + "/hz", ui.value);
	    });
	  }
	  ...
	  for (var i = 0; i < 32; i++) {
	    $('#' + crate.portgroup_name + '_effects_eq').append
       ("<div class='kradmixer_control' id='"
       + crate.portgroup_name + "_eq_" + i + "_db'></div>");
	    $('#' + crate.portgroup_name + '_eq_'
       + i + '_db').slider({orientation: 'vertical',
       value: crate.eq.bands[i].db, step: 0.1, min: -12.0, max: 12.0 });
	    $('#' + crate.portgroup_name + '_eq_' + i + '_db' ).bind
       ("slide", {band: i}, function(event, ui) {
	      kradradio.ctrl("e/" + crate.portgroup_name + "/eq/" + event.data.band + "/db", ui.value);
	    });
	  }
	  ...
	  for (var i = 0; i < 32; i++) {
	    $('#' + crate.portgroup_name + '_effects_eq').append
       ("<div class='kradmixer_control' id='"
       + crate.portgroup_name + "_eq_" + i + "_bw'></div>");
	    $('#' + crate.portgroup_name + '_eq_' + i + '_bw').slider
       ({orientation: 'vertical', value: crate.eq.bands[i].bw,
       step: 0.1, min: 0.1, max: 5.0 });
	    $('#' + crate.portgroup_name + '_eq_' + i + '_bw' ).bind
       ("slide", {band: i}, function(event, ui) {
	      kradradio.ctrl("e/" + crate.portgroup_name
         + "/eq/" + event.data.band + "/bw", ui.value);
	    });
	  }
  }
  ...
  if ($.browser.webkit) {
	  $('.krad_button_small').css('background',
     '-webkit-radial-gradient(center, ellipse cover, '
     + 'rgba(151,151,151,1) 12%,rgba(171,171,171,1) 100%)');
	  $('.krad_button_small:active').css('background',
     '-webkit-radial-gradient(center, ellipse cover, '
     + 'rgba(241,231,103,1) 0%,rgba(254,182,69,1) 100%)');
	  $('.krad_button').css('background',
     '-webkit-radial-gradient(center, ellipse cover, '
     + 'rgba(241,231,103,1) 12%,rgba(254,182,69,1) 100%)');
	  $('.krad_button:active').css('background',
     '-webkit-radial-gradient(center, ellipse cover, '
     + 'rgba(241,231,103,1) 12%,rgba(254,182,69,1) 100%)');
    $('.krad_button2').css('background',
     '-webkit-radial-gradient(center, ellipse cover, '
     + 'rgba(83,203,241,1) 0%,rgba(5,171,224,1) 100%)');
	  $('.krad_button2:active').css('background',
     '-webkit-radial-gradient(center, ellipse cover, '
     + 'rgba(83,203,241,1) 0%,rgba(5,171,224,1) 100%)');
  }
}
...
Kradradio.prototype.got_add_decklink_device = function() {
}

Kradradio.prototype.got_remove_link = function(crate) {
}

Kradradio.prototype.got_add_link = function(crate) {
}
