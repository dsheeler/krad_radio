...
Shape.prototype.draw = function(ctx, fillColor, onGhost) {
	...
	ctx.lineTo(this.x+0.5*this.w, this.y-0.25*this.h);
	ctx.stroke();
	// draw the boxes
	...
	selectionHandles[8].y = this.y-0.25*this.h;
    ...
    var tmpy = this.y;
    ctx.fillText(this.label, this.x, tmpy);
    tmpy += lineHeight;
...
function CanvasState(canvas, ghostcanvas) {
	...
	switch (myState.expectResize) {
	  case 0:
	      ...
	      kradradio.update_sprite(myState.selection.num, "xs", xs);
	      kradradio.update_sprite(myState.selection.num, "x", mx);
	    
	      myState.selection.xs = xs;
	      myState.selection.x = mx;

	      var ys = (myState.selection.ys * myState.selection.h + oldy - my) / myState.selection.h ;
	      kradradio.update_sprite(myState.selection.num, "ys", ys);
	      kradradio.update_sprite(myState.selection.num, "y", my);
	  ...
	  case 2:
	      ...
	      kradradio.update_sprite(myState.selection.num, "xs", (mx - oldx) / myState.selection.w);
	  ...
	  case 5:
	      ...
	      kradradio.update_sprite(myState.selection.num, "ys", (my - oldy) / myState.selection.h);
	  ...
	  case 8:
	    var angle = Math.atan2((mx - (myState.selection.x+0.5*myState.selection.w)), -(my - (myState.selection.y+0.5*myState.selection.h)));
	    kradwebsocket.debug(angle);
	    myState.selection.r = angle;
	    kradradio.update_sprite(myState.selection.num, "r", angle *180/Math.PI);
	    break;

	  case 9:
      	    kradradio.update_sprite(myState.selection.num, "o", (myState.selection.h-mouse.y)/myState.selection.h);
	    myState.selection.o = 2*(myState.selection.h-mouse.y) / myState.selection.h;
	    break;
	  }
