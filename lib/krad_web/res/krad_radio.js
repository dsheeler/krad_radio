...
Kradradio.prototype.crate_handler = function (crate) {
  if (crate.com == "kradmixer") {
    ...
    if (crate.cmd == "add_portgroup") {
      kradradio.got_add_portgroup (crate.portgroup_name, crate.volume,
                                   crate.crossfade_name, crate.crossfade, crate.xmms2 );
      return;
    }
...
Kradradio.prototype.got_add_portgroup = function (portgroup_name, volume, crossfade_name, crossfade, xmms2) {
  ...
  volume_div = "<div class='kradmixer_control volume_control' id='portgroup_" + portgroup_name + "_wrap'>\
                <div class='meter_wrapper'><canvas width='42' height='200' class='meter' id='" + portgroup_name + "_meter_canvas'></canvas></div>\
                <div id='" + portgroup_name + "'></div><h2>" + portgroup_name + "</h2></div>";
  ...
  if (crossfade_name.length > 0) {
    kradradio.portgroup_handle_crossfade_group(portgroup_name, crossfade_name, crossfade);
  }
  
  if (xmms2 == 1) {
    kradradio.portgroup_handle_xmms2(portgroup_name, xmms2);
  }

  $('#portgroup_' + portgroup_name + '_wrap').append("<div id='ktags_" + portgroup_name + "'></div>");

  this.portgroup_draw_peak_meter (portgroup_name, 0);
