...
//            mx = (mx - myState.selection.x) / myState.selection.x.xs; 
            var mtx =   Math.cos(myState.selection.r) * (mx - Rcx) + Math.sin(myState.selection.r) * (my - Rcy);
            var mty =   -Math.sin(myState.selection.r) * (mx - Rcx) + Math.cos(myState.selection.r) * (my - Rcy);
                          
            var seltx = Math.cos(myState.selection.r) *  (myState.selection.x - Rcx) 
                - Math.sin(myState.selection.r) * (myState.selection.y - Rcy);
            var selty = Math.sin(myState.selection.r) * (myState.selection.x - Rcx) 
               + Math.cos(myState.selection.r) * (myState.selection.y - Rcy);


            myState.selection.PCView.handles[myState.expectMovePerspectiveCorrectionHandle].x =  
                (mtx + Rcx) / myState.selection.xs - myState.selection.x / myState.selection.xs;
            myState.selection.PCView.handles[myState.expectMovePerspectiveCorrectionHandle].y =  
                (mty + Rcy) / myState.selection.ys - myState.selection.y / myState.selection.ys;
