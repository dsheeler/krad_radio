...
function Shape(label, x, y, w, h, r, o, xs, ys, fill) {
...
//  ctx.translate(this.w*0.5*this.xs, this.h*0.5*this.ys);
...
//    myState.keypressed = true;
//    kradwebsocket.debug('pressed');
//  });

//  canvas.keyup( function(e) {
//    myState.keypressed = false;
//  });
  ...
  canvas.addEventListener('mousedown', function(e) {
	...
	myState.selection.num = i;
	kradwebsocket.debug(myState.selection.num);
        myState.valid = false;
        return;
      }
    }
  ...
  canvas.addEventListener('mousemove', function(e) {
	...
	if (kradradio.compositor_mode == "Position") {
	    ...
	    kradradio.update_sprite(myState.selection.num, "r", angle);//360.0/Math.PI * Math.atan((mouse.y - 0.5*(myState.selection.y+myState.selection.h))/(mouse.x - 0.5*(myState.selection.x+myState.selection.w))));
	} else if (kradradio.compositor_mode == "Opacity") {
	    kradradio.update_sprite(myState.selection.num, "o", (myState.selection.h-mouse.y)/myState.selection.h);
	 } else if (kradradio.compositor_mode == "Scale") {
	     if ((myState.dragoffx > (myState.selection.w - 10)) && (myState.dragoffx < (myState.selection.w + 10))) {
		 myState.selection.xs = (mouse.x - myState.selection.x)/myState.selection.w;		 
		 kradwebsocket.debug(myState.selection.xs);
	     }

	     if ((myState.dragoffy > myState.selection.h - 10) && (myState.dragoffy < myState.selection.h + 10)) {
		 myState.selection.ys = (mouse.y - myState.selection.y)/myState.selection.h;		 
	     }


	    kradradio.update_sprite(myState.selection.num, "ys", myState.selection.ys);
	    kradradio.update_sprite(myState.selection.num, "xs", myState.selection.xs);
	}
...
Kradradio.prototype.got_sysname = function (sysname) {
  ...
  $('#kradradio').append("<div class='kradradio_station' id='" + this.sysname + "'>\
    ...
    <canvas height='200' width='84' class='meter' id='MasterBUS_meter_canvas'></canvas>\
    <h2>MasterBus</h2>\
    </div>\
    </div>\
    <br clear='both'>\
...
Kradradio.prototype.crate_handler = function (crate) {
	...
	if (crate.cmd == "add_sprite") {
	    kradradio.got_add_sprite ( crate );
	    return;
	}  
...
Kradradio.prototype.update_sprite = function (sprite_num, control_name, value) {
...
Kradradio.prototype.got_add_sprite = function (crate) {
...
*/
  ...
  if (! document.getElementById(crate.portgroup_name + "_sprite_canvas")) {
    ...
    kradradio.compositor_canvas = new CanvasState(document.getElementById(crate.portgroup_name + "_sprite_canvas"));
  }

  kradradio.compositor_canvas.addShape(new Shape("Sprite " + crate.sprite_num, crate.x,crate.y, crate.w, crate.h)); // The default is gray
