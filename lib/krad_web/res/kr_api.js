  ...
  if (this.connected != true) {
    ...
    if (this.reconnection_attempts == 3) {
      $('#kradradio').append("<div id='websockets_connection_problem'>"
       + "<h2>Websockets connection problem using port " + this.ws_port 
       + "</h2></div>");
    }
    this.ws_connect();
  }
...
Kr.prototype.got_messages = function(msgs) {
...
function Kr() {
  ...
  this.ws_port = window.location.port;
  this.ws_uri = 'ws://' + location.hostname + ':' + this.ws_port + '/';
  this.ws = "";
  this.ws_reconnection_attempts = 0;
  this.ws_reconnect = false;  
  this.ws_connected = false;
