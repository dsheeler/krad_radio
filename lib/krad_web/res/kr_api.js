...
Tag.prototype.show = function() {
  ...
  if (this.tag_item == "station") {
    ...
    if (this.tag_name.indexOf("_bool") != -1) {
      $('.kradradio_tools').append("<div class='button_wrap'>"
       + "<div class='krad_button_toggle' id='" + this.tag_name + "'>" 
       + this.tag_name.replace("_bool","").replace("_", " ") + "</div>");
      $('.kradlink_tools').append("<br clear='both'/>");  
          
      kradradio.add_bool(this.tag_name);
      kradradio.got_set_bool(this.tag_name, this.tag_value);

      $('#' + this.tag_name).bind("click", 
       {bool: this.tag_name}, 
       function(event, ui) {
      ...
      $('.station_tags').append("<div id='ktag_" + this.tag_num 
       + "' class='tag'>" + this.tag_item + ": " + this.tag_name 
       + " - <span class='tag_value'>" + this.tag_value + "</span></div>");    
    }

  } else {
    if ((this.tag_name != "artist") && (this.tag_name != "title")) {
      if (this.tag_name == "now_playing") {
        $('#ktags_' + this.tag_item).append("<div id='ktag_" 
         + this.tag_num + "' class='tag nowplaying'><span class='tag_value'>" 
         + this.tag_value + "</span></div>");
        return;
      }
      if (this.tag_name == "playtime") {
        $('#ktags_' + this.tag_item).append("<div id='ktag_" 
         + this.tag_num + "' class='tag playtime'><span class='tag_value'>"
         + this.tag_value + "</span></div>");
        return;
      }  
      $('#ktags_' + this.tag_item).append("<div id='ktag_" 
       + this.tag_num + "' class='tag'>" + this.tag_item + ": "
       + this.tag_name + " - <span class='tag_value'>"
       + this.tag_value + "</span></div>");          
    }
  }
}
