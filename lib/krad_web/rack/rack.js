...
Rack.prototype.position_units = function(auto) {
...
Rack.prototype.on_crate = function(crate) {
  if (crate.com == "kradmixer") {
    ...
    if (crate.ctrl == "update_portgroup") {
      if (this.units[i].title == 'Xmms2') {
        kr.debug("UPDATE_PORTGROUP: " + this.units[i].title + " " + this.units[i].description);
        kr.debug('this is here, ho');
        this.units[i].got_update_portgroup(crate.portgroup_name,
        crate.control_name, crate.value);
      }
    }
    if (crate.ctrl == "add_portgroup") {
      kr.debug("ADD_PORTGROUP: " + this.units[i].title + " " + this.units[i].description);
      if (this.units[i].title == 'Xmms2' || this.units[i].title == 'Mixer'
       || this.units[i].title == 'Effects') {
