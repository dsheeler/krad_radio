...
Mixer.prototype.got_add_portgroup = function(crate) {
  ...
  $('#mixers .minimized_portgroup_wraps').append("<div "
   ...
   + "id='portgroup_" + pname + "_minimized_wrap'><h3>"
   + pname + "</h3></div>");

  $("#portgroup_" + pname + "_wrap .close").bind('mouseenter', {'crate': crate}, function(event) {
    var selectors = this.maximized_crossfader_group_selectors(event.data.crate.content.name);
   ...
   $("#portgroup_" + pname + "_wrap .close").bind('mouseleave', {'crate': crate}, function(event) {
    var selectors = this.maximized_crossfader_group_selectors(event.data.crate.content.name);
    for (var i = 0; i < selectors.length; i++) {
  ...
  $("#portgroup_" + pname + "_wrap .close").bind('click', {'crate': crate}, function(event) {

    var selectors = this.maximized_crossfader_group_selectors(event.data.crate.content.name);
    var min_selectors = this.minimized_crossfader_group_selectors(event.data.crate.content.name);
    for (var i = 0; i < selectors.length; i++) {
    ...
    if (min_selectors.length == 1) {
      $("#portgroup_" + event.data.crate.contente.name +"_minimized_wrap").append($("#"
       + event.data.crate.content.name + "_meter_canvas"));

      $("#" + event.data.crate.content.name + "_meter_canvas").css({'position': 'relative',
         'display': 'inline'});

      $('#portgroup_' + event.data.crate.content.name  + '_minimized_wrap').bind('click', {'crate': event.data.crate}, function(e) {

        $('#portgroup_' + e.data.crate.content.name  + '_wrap').show(200);

        $("#portgroup_" + e.data.crate.content.name + "_wrap .meter_wrapper" ).append(
         $("#" + e.data.crate.content.name + "_meter_canvas"));

        $("#" + e.data.crate.content.name + "_meter_canvas").css({'top': '0px'});
      ...
      for (var i = 0; i < 2; i++) {
        $("#portgroup_" + this.crossfades[event.data.crate.content.name][i] +"_minimized_wrap").append($("#"
         + this.crossfades[event.data.crate.content.name][i] + "_meter_canvas"));

        $("#" + this.crossfades[event.data.crate.content.name][i] + "_meter_canvas").css({'position': 'relative',
          'display': 'inline'});

        $('#portgroup_' + this.crossfades[event.data.crate.content.name][i]  + '_minimized_wrap').bind('click', {'i': i, 'crate': event.data.crate}, function(e) {
          var selectors = this.maximized_crossfader_group_selectors(e.data.crate.content.name);
          var min_selectors = this.minimized_crossfader_group_selectors(e.data.crate.content.name);
          for (var j = 0; j < selectors.length; j++) {
            $(selectors[j]).show(200);
          }
          for (var j = 0; j < 2; j++) {
            $("#portgroup_" + this.crossfades[e.data.crate.content.name][j] + "_wrap .meter_wrapper" ).append(
             $("#" +  this.crossfades[e.data.crate.content.name][j] + "_meter_canvas"));
            $("#" + this.crossfades[e.data.crate.content.name][j] + "_meter_canvas").css({'top': '0px'});
          }
  ...
  $('#eq_' + pname).bind("click", { 'name': pname },
   function(event) {
     kr.ux.share('eq_display', event.data.name);
   });

  $('#fx_' + pname).bind("click", { 'name': pname },
   function(event) {
     kr.ux.share('effects_display', event.data.name);
   });

  var me = this;
  $('#fade_' + pname).bind("click", {'me': me},
    function(event) {
     var new_value = 0;
     if (event.data.me.portgroup_volume_toggle[pname]) {
       new_value = event.data.me.portgroup_volume_toggle[pname];
       event.data.me.portgroup_volume_toggle[pname] = 0;
     } else {
       new_value = 0;
       event.data.me.portgroup_volume_toggle[pname]
        = event.data.me.portgroup_values[pname];
     }
     kr.ctrl("m/" + pname, new_value, 1500);
  });

  var name = pname;
  var volume_element = document.id("portgroup_" + pname + "_wrap");
  var volume_slider = volume_element.getElement('.volume');

  this.portgroup_values[pname] = crate.content.volume[0];
  this.portgroup_volume_sliders[name] = new Slider(volume_slider,
   volume_slider.getElement('.handle'), {
     ...
     wheel: true,
     mode: "vertical",
     initialStep: crate.content.volume[0],
     onChange: function (value) {
       kr.ctrl("m/" + pname, value)
     }
  });

  this.draw_peak_meter(pname, 0);
}
