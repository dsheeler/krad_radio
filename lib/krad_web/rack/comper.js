Kr.prototype.comper_got_add_subunit = function(crate) {
...
Kr.prototype.comper_got_remove_subunit = function(crate) {
  this.compositor_canvas.removeShape(crate.subunit_type, crate.subunit_id);
  this.compositor_canvas.valid = false;
}

Kr.prototype.comper_update_subunit = function(num, type, control_name, value) {
...
Shape.prototype.draw = function(ctx, onGhost) {
  ...
  if (this.selected == true) {
    var half = this.selectionBoxSize / 2;
	  ...
	  kr.ux.selectionHandles[0].x = this.x;
	  kr.ux.selectionHandles[0].y = this.y;

	  kr.ux.selectionHandles[1].x = this.x+this.w/2;
	  kr.ux.selectionHandles[1].y = this.y;

	  kr.ux.selectionHandles[2].x = this.x+this.w;
	  kr.ux.selectionHandles[2].y = this.y;

	  /*Middle left handle.*/
	  kr.ux.selectionHandles[3].x = this.x;
	  kr.ux.selectionHandles[3].y = this.y+this.h/2;

	  /*Middle right handle.*/
	  kr.ux.selectionHandles[4].x = this.x+this.w;
	  kr.ux.selectionHandles[4].y = this.y+this.h/2;

    /*Bottom left, middle, and right handles.*/
    kr.ux.selectionHandles[6].x = this.x+this.w/2;
    kr.ux.selectionHandles[6].y = this.y+this.h;

    kr.ux.selectionHandles[5].x = this.x;
    kr.ux.selectionHandles[5].y = this.y+this.h;

    kr.ux.selectionHandles[7].x = this.x+this.w;
    kr.ux.selectionHandles[7].y = this.y+this.h;

    /*Rotation handle.*/
    kr.ux.selectionHandles[8].x = this.x+0.5*this.w;
    kr.ux.selectionHandles[8].y = this.y- 3*this.selectionBoxSize;

    /*Opacity handle.*/
    kr.ux.selectionHandles[9].x = this.x+0.5*this.w;
    kr.ux.selectionHandles[9].y = this.y+0.5*this.h;

    for (var i = 0; i < 10; i ++) {
      var cur = kr.ux.selectionHandles[i];
      ctx.beginPath();
...
function CanvasState(canvas, ghostcanvas) {
  ...
  canvas.addEventListener('mousemove', function(e) {
      ...
      myState.selection.x = mouse.x - myState.dragoffx;
	    ...
	    kr.ux.comp_ctrl(myState.selection.num,
      ...
      switch (myState.expectResize) {
          ...
          kr.ux.comp_ctrl(myState.selection.num, myState.selection.type, "xscale", xscale);
          kr.ux.comp_ctrl(myState.selection.num, myState.selection.type, "x", x);

          myState.selection.xs = xscale;
          myState.selection.x = x;

          kr.ux.comp_ctrl(myState.selection.num, myState.selection.type, "yscale", xscale);
          kr.ux.comp_ctrl(myState.selection.num, myState.selection.type, "y", y);
        ...
        case 1:
           ...
           / myState.selection.h ;
	  	    kr.ux.comp_ctrl(myState.selection.num, myState.selection.type,
           "yscale", ys);
          kr.ux.comp_ctrl(myState.selection.num, myState.selection.type,
           "y", my);
