...
Comper.prototype.shared = function(key, shared_object) {
  if (key == 'selected') {
    var id = shared_object.selected.id;
    var type = shared_object.selected.type;
    var alreadySelected = shared_object.selected.alreadySelected;

    var shapes = this.compositor_canvas.shapes;
    
    var l = shapes.length;

	  for (var i = 0; i < l;  i++) {
	    shapes[i].selected = false;
	    if (shapes[i].num == id && shapes[i].type == type) {
		    shapes[i].selected = !alreadySelected;
        if (!alreadySelected) {
          this.compositor_canvas.selection = shapes[i];
        }
      }
    }
