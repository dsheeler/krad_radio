...
function LocalStream(name, stream, containerDiv, divName, canvasId) {

  this.name = name;
  this.divName = divName;
  this.canvasId = canvasId;

  containerDiv.append('<div class="local_stream_div" id="' + this.divName
   + '"></div>');

  var local_stream_div = $('#' + this.divName);
  var local_stream_id = "local_stream_" + this.name;
  ...
  local_stream_div.append('<video class="local_stream_video" id="'
   + vidName + '" autoplay muted></video>');

  this.videoDiv = document.querySelector('#' + vidName);
  this.videoDiv.addEventListener('playing', function() {
    kr.rtc.handleLocalVideoStream(vidName, local_stream_div);
  });
...
Rtc.prototype.handleLocalVideoStream = function(vidName, local_stream_div) {
  if ($('#' + vidName).height() != 150) {
      ...
      local_stream_div.css('width', $('#' + vidName).outerWidth());
      local_stream_div.css('height', $('#' + vidName).outerHeight());
  } else {
    setTimeout(function() {
      kr.rtc.handleLocalVideoStream(vidName, local_stream_div);
    }, 100);
...
Rtc.prototype.bell = function() {
  ...
  playTone(120, 3/120.0, kr.rtc.audio_context);
  playTone(200, 2/200.0, kr.rtc.audio_context);
  playTone(333, 2/333.0, kr.rtc.audio_context);
  playTone(500, 10/500.0, kr.rtc.audio_context);
  playTone(800,  8/800.0, kr.rtc.audio_context);
  playTone(1100, 5/1100.0, kr.rtc.audio_context);
  playTone(2000, 20/2000.0, kr.rtc.audio_context);
  playTone(1500, 30/1500.0, kr.rtc.audio_context);
  playTone(2500, 12/2500.0, kr.rtc.audio_context);
}
...
function handleUserMedia(stream) {
  ...
  kr.rtc.localStreams.push(new LocalStream(streamName, stream, 
...
function handleRemoteStreamAdded(event) {
  ...
  $('#remoteVideoContainer').append('<div id="' + call_container_id
   ...
   + '</p><video id="' + streamId
   + '" class="remote_video" autoplay></video>' 
   + '<canvas class="local_video_canvas" id="' + remoteCanvasId 
   + '"></canvas></div>');

  var sBox = new SpectrumBox(2048, 32, remoteCanvasId, kr.rtc.audio_context,
  SpectrumBox.Types.FREQUENCY);
  var ctx = sBox.getCanvasContext();
  ctx.fillStyle = 'rgb(50,100,200)';
  ...
  var remoteVideoElem = document.querySelector('#' + streamId);
  //remoteVideoElem.addEventListener('loadedmetadata', function() {
  ...
  for(var i = 0; i < kr.rtc.localStreams.length; i++) {
    ...
    var sBox = new SpectrumBox(2048, 128, canvasId, kr.rtc.audio_context,
     SpectrumBox.Types.FREQUENCY);
    var ctx = sBox.getCanvasContext();
    ctx.fillStyle = 'rgb(50,100,200)';
    var microphone = kr.rtc.audio_context.createMediaStreamSource(
    ...
    $('#' + local_video_div_id).resizable({
      ...
      stop: function(e, ui) {
        var w = $('#' + call_container_id).outerWidth();
        var h = $('#' + call_container_id).outerHeight();
    ...
    remoteVideoElem.addEventListener('playing', function() {
      setTimeout(kr.rtc.handleRemoteVideoPlaying(streamId, 
...
Rtc.prototype.handleRemoteVideoPlaying = function(
 streamId, call_container_id,
 local_video_div_id, vidId, canvasId) {
  var vid_height = $('#' + streamId).height()
  if ((vid_height != 0) && (vid_height == 720)) {
    $('#' + call_container_id).css('height', $('#' + streamId).outerHeight());

    var w = $('#' + call_container_id).outerWidth();
    var h = $('#' + call_container_id).outerHeight();
    $('#' + local_video_div_id).css('height',
     100 * $('#' + vidId).outerHeight()/h + '%');
    w = $('#' + local_video_div_id).width();
    $('#' + vidId).css('width', 100 * $('#' + vidId).width()/w + '%');
    $('#' + canvasId).css('width', 100 * $('#' + vidId).outerWidth()/w + '%');
  } else {
    setTimeout(function() {
      kr.rtc.handleRemoteVideoPlaying(streamId, 
       call_container_id, local_video_div_id, vidId, canvasId);
    }, 500);
  }
}
