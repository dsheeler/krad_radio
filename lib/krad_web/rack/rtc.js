...
function Rtc(info_object) {
  ...
  <button id='start'>start</button><button id='call'>Call</button>\
  </div>");
  ...
  $('#start').bind('click', {'me': this}, function(e) {
    e.data.me.startMedia();
  //  e.data.me.maybeStart();
  });

  kr.rtc = this;

  window.onbeforeunload = function(){
    kr.ux.sendMessage('bye');
  }
...
function handleUserMedia(stream) {
  ...
  kr.rtc.localVideo.src = window.URL.createObjectURL(stream);
...
Rtc.prototype.createPeerConnection = function() {
  try 
    {
      var peerConnection = typeof(webkitRTCPeerConnection) == 'undefined' ? mozRTCPeerConnection : webkitRTCPeerConnection;
...
function handleRemoteStreamAdded(event) {
  ...
  kr.rtc.remoteVideo.src = window.URL.createObjectURL(event.stream);
