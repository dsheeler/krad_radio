...
Rtc.prototype.ring = function() {
  playTone(80, this.audio_context);
  playTone(120, this.audio_context);
  playTone(200, this.audio_context);
  playTone(333, this.audio_context);
  playTone(500, this.audio_context);
  playTone(800, this.audio_context);
  playTone(1100, this.audio_context);
  playTone(2000, this.audio_context);
  playTone(1500, this.audio_context);
  playTone(2500, this.audio_context);
}
...
function playTone(freq, ctx) {
  ...
  g.gain.linearRampToValueAtTime(0.0, ctx.currentTime+0.24);
}
...
Rtc.prototype.update = function(crate) {
  ...
  if (crate.ctrl == 'call') {
    ...
    this.ring();
    var r = confirm("Call from: '" + crate.user + "'. Answer?");
    if (r == true) {
      for (var i = 0; i < this.peers.length; i++) {
