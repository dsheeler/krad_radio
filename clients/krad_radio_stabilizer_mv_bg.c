...
int videoport_process_out(void *buffer, void *user) {
  ...
  dst_x = snapshot->dbox.rect.x - snapshot->bbox.x;
  dst_y = snapshot->dbox.rect.y - snapshot->bbox.y;
  ...
  cairo_rectangle(cr, 0, 0, snapshot->width, snapshot->height);
  cairo_fill(cr);
  cairo_restore(cr);
  /* Teardown */
  cairo_destroy (cr);
...
int videoport_process_in (void *buffer, void *user) {
  ...
  if (snapshot->got_frame == 0) {
    ...
    if (first_call == 1) {
      first_call = 0;
      ccv_read(snapshot->rgba, &x, CCV_IO_ARGB_RAW | CCV_IO_GRAY, snapshot->height, snapshot->width, snapshot->width * 4);
