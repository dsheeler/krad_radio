...
#include "/home/oneman/kode/libav/libavutil/colorspace.h"
...
static void send_frame_to_krad (AVFrame *src_frame, float last_frame_ms) {
  ...
  if ((kplayer.old_src_w != src_frame->width) ||
      (kplayer.old_src_h != src_frame->height) ||
      (kplayer.old_px_fmt != src_frame->format)) {
 
    if (kplayer.sws_converter != NULL) {
        sws_freeContext ( kplayer.sws_converter );
    }
    kplayer.sws_converter = NULL;
  }
  
  
  if (kplayer.sws_converter == NULL) {
  
    kplayer.old_src_w = src_frame->width;
    kplayer.old_src_h = src_frame->height;
    kplayer.old_px_fmt = src_frame->format;

  
    kplayer.sws_converter =
      sws_getContext ( src_frame->width,
               src_frame->height,
               src_frame->format,
               960,
               540,
...
int videoport_process (void *buffer, void *arg) {
  ...
  if (kplayer.updated == 1) {
    ...
    if (kplayer.updated2 == 1) {
      memcpy (buffer, kplayer.rgba2, 960 * 540 * 4);
      kplayer.updated2 = 0;
    }
