#include "kr_x11_stream_config.h"
#include "../kr_config/kr_config.c"

int handle_config(struct kr_x11_stream_params *params, char *configname) {
  kr_cfg *config;
  ...
  if (config->get_val(config, "window_id")) {
    params->window_id = atoi(config->value);
  }
